...
- **`http_server.py`** — Flask REST API; uses a `create_app()` factory that accepts dependency injection for testing. Served via `waitress` (not Flask's dev server); `_run_server` handles SIGTERM/SIGINT and turns the strip off on exit
