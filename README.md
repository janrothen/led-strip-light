...
- Multiple built-in LED effects (breathing, fade, rainbow, random, campfire, candle, aurora, heartbeat, lightning, and more)
...
graph TD
    ...
    subgraph LED Control
        ...
        ER["EffectRunner\n(effect dispatch)"]
        EFX["effects.py\n(breathing · fade · campfire\ncandle · aurora · heartbeat · lightning\nrainbow · cycle · random)"]
    end
...
# Lightning (random bright flashes with fast decay)
./run.py lightning --max-gap-ms 5000
...
- **Features:**
  ...
  - Start/stop built‑in effects (breathing, campfire, candle, aurora, heartbeat, lightning, rainbow, random, fade) with parameter inputs
...
* `POST /effects/lightning` (optional JSON overrides: flash_color, background_color, min_gap_ms, max_gap_ms, flash_ms, intensity_min, intensity_max, aftershock_chance, max_aftershocks, duration, gamma)
...
# Lightning with cyan flashes and shorter gaps
curl -X POST -H 'Content-Type: application/json' \
  -d '{"flash_color":"00FFFF","max_gap_ms":3000}' \
  http://localhost:5000/effects/lightning
